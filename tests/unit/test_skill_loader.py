...
from repo_claude.core.skills.loader import Skill, SkillLoader
...
def test_frontmatter_parsed(tmp_path: Path) -> None:
    from repo_claude.core.skills.loader import _parse_skill_file
...
# 设计：在 .repo/skills/ 中写入同名文件，用 monkeypatch 修改 cwd，断言加载到的是本地版本
def test_project_overrides_global(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    local_skills = tmp_path / ".repo" / "skills"
