...
def test_dotenv_base_loaded(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "REPO_PORT=9999\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("REPO_PORT", raising=False)
...
def test_system_env_overrides_dotenv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _write_env(env_file, "REPO_PORT=9999\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("REPO_PORT", "8888")
...
# 功能：验证 .env 中设置的 REPO_CONFIG 能正确影响 TOML 配置文件的加载路径
# 设计：.env 指向自定义 TOML 文件，TOML 中写入不同端口，确认 .env 在 TOML 加载前被读取（优先级链的正确顺序）
def test_dotenv_before_toml_repo_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "custom.toml"
    toml_path.write_bytes(b'[core]\nport = 5555\n')

    env_file = tmp_path / ".env"
    _write_env(env_file, f"REPO_CONFIG={toml_path}\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("REPO_CONFIG", raising=False)
    monkeypatch.delenv("REPO_PORT", raising=False)
...
def test_priority_chain_full(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # .env：7000
    # 系统环境变量：8000（最高）
    toml_path = tmp_path / "repo.toml"
    toml_path.write_bytes(b'[core]\nport = 6000\n')

    env_file = tmp_path / ".env"
    _write_env(env_file, "REPO_PORT=7000\n")

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("REPO_CONFIG", str(toml_path))
    monkeypatch.setenv("REPO_PORT", "8000")
