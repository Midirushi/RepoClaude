...
# Run daemon manually
uv run repo-core                        # foreground; Ctrl+C to stop
REPO_PORT=8000 uv run repo-core        # override port
...
This is a **dual-process** local AI agent system. `repo-core` is a persistent daemon; `repo` and `repo-tui` are clients that connect to it over a Unix domain socket.

```
repo-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
repo (CLI)   repo-tui (TUI, S2+)
```

**`repo-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `repo` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/repo_claude/core/bus/`)
...
### Transport layer (`src/repo_claude/core/transport/`)
...
### Config (`src/repo_claude/core/config.py`)

Four-tier priority: **built-in defaults → `~/.repo/config.toml` → `.env` → env vars**.
...
Relevant env vars: `REPO_CONFIG`, `REPO_HOST`, `REPO_PORT`, `REPO_LOG_LEVEL`, `REPO_LOG_FILE`, `REPO_LOG_FORMAT`.

### Daemon entry (`src/repo_claude/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `REPO_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
