...
_PID_FILE = Path.cwd() / ".repo-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: RepoConfig) -> None:
...
def cmd_core_status(config: RepoConfig) -> None:
    try:
...
def cmd_core_start(config: RepoConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "repo_claude.core"],
...
def cmd_core_stop(config: RepoConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
