...
import repo_claude
from repo_claude.core.bus.commands import PongResult
from repo_claude.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from repo_claude.core.config import RepoConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: RepoConfig) -> None:
    try:
...
async def _ping(config: RepoConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{repo_claude.__version__}"},
    }
