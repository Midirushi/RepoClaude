...
_DEFAULT_LOG_FILE = "~/.repo/logs/core.log"
_DEFAULT_LOG_FORMAT = "text"
_DEFAULT_CONFIG_PATH = "~/.repo/config.toml"
_DEFAULT_MAX_STEPS = 20
_DEFAULT_MODEL = "claude-sonnet-4-6"
_DEFAULT_TRACE_FILE = "~/.repo/traces/daemon.jsonl"
...
def get_config() -> RepoConfig:
    config = RepoConfig()

    # .env 必须在读取 REPO_CONFIG 之前加载，以便 .env 中的 REPO_CONFIG 能影响 TOML 路径
    # 尝试多个可能的 .env 路径（支持从任意目录启动）
    env_paths = [
        Path(".env"),  # 当前工作目录
        Path(__file__).parent.parent.parent.parent / ".env",  # 项目根目录
        Path.cwd() / ".env",  # 当前工作目录（绝对路径）
    ]
    # 项目级覆盖键：即使 shell 已 export，也以 .env 为准（避免 ~/.zshrc 等全局配置干扰）
    ...
    # 若显式指定 REPO_CONFIG，只读该文件；否则按优先级叠加：全局 → 项目本地
    explicit = os.environ.get("REPO_CONFIG")
    if explicit:
...
def _apply_toml(config: RepoConfig, data: dict[str, Any]) -> None:
...
# 用 REPO_* 环境变量覆盖 config 中对应字段（若变量已设置）
def _apply_env(config: RepoConfig) -> None:
    host = os.environ.get("REPO_HOST")
    if host is not None:
        config.host = host

    port_str = os.environ.get("REPO_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(f"Config error: REPO_PORT must be an integer, got: {port_str!r}")

    log_level = os.environ.get("REPO_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("REPO_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("REPO_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format

    max_steps_str = os.environ.get("REPO_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: REPO_MAX_STEPS must be a positive integer,"
                    f" got: {max_steps_str!r}"
                )
            config.agent.max_steps = val
        except ValueError:
            raise SystemExit(
                f"Config error: REPO_MAX_STEPS must be an integer, got: {max_steps_str!r}"
            )

    default_model = os.environ.get("REPO_LLM_DEFAULT_MODEL")
    if default_model is not None:
        config.llm.default_model = default_model

    trace_enabled = os.environ.get("REPO_TRACE_ENABLED")
    if trace_enabled is not None:
        config.trace.enabled = trace_enabled.lower() not in ("0", "false", "no")

    trace_file = os.environ.get("REPO_TRACE_FILE")
    if trace_file is not None:
        config.trace.file = trace_file

    trace_payload = os.environ.get("REPO_TRACE_INCLUDE_LLM_PAYLOAD")
    if trace_payload is not None:
        config.trace.include_llm_payload = trace_payload.lower() not in ("0", "false", "no")

    perm_timeout = os.environ.get("REPO_PERMISSION_TIMEOUT_S")
    if perm_timeout is not None:
        try:
            perm_timeout_val = float(perm_timeout)
            if perm_timeout_val < 0:
                raise SystemExit(
                    f"Config error: REPO_PERMISSION_TIMEOUT_S must be >= 0, got: {perm_timeout!r}"
                )
            config.permission.timeout_s = perm_timeout_val
        except ValueError:
            raise SystemExit(
                f"Config error: REPO_PERMISSION_TIMEOUT_S must be a number, got: {perm_timeout!r}"
            )

    compact_threshold = os.environ.get("REPO_COMPACT_THRESHOLD")
    if compact_threshold is not None:
        try:
            compact_threshold_val = float(compact_threshold)
            if not (0.0 <= compact_threshold_val <= 1.0):
                raise SystemExit(
                    f"Config error: REPO_COMPACT_THRESHOLD must be between 0 and 1, got: {compact_threshold!r}"
                )
            config.compaction.auto_threshold = compact_threshold_val
        except ValueError:
            raise SystemExit(
                f"Config error: REPO_COMPACT_THRESHOLD must be a number, got: {compact_threshold!r}"
            )

    compact_tool_limit = os.environ.get("REPO_COMPACT_TOOL_LIMIT")
    if compact_tool_limit is not None:
        try:
            compact_tool_limit_val = int(compact_tool_limit)
            if compact_tool_limit_val <= 0:
                raise SystemExit(
                    f"Config error: REPO_COMPACT_TOOL_LIMIT must be a positive integer, got: {compact_tool_limit!r}"
                )
            config.compaction.tool_result_limit = compact_tool_limit_val
        except ValueError:
            raise SystemExit(
                f"Config error: REPO_COMPACT_TOOL_LIMIT must be an integer, got: {compact_tool_limit!r}"
            )

    compact_tool_keep = os.environ.get("REPO_COMPACT_TOOL_KEEP")
    if compact_tool_keep is not None:
        try:
            compact_tool_keep_val = int(compact_tool_keep)
            if compact_tool_keep_val <= 0:
                raise SystemExit(
                    f"Config error: REPO_COMPACT_TOOL_KEEP must be a positive integer, got: {compact_tool_keep!r}"
                )
            config.compaction.tool_result_keep = compact_tool_keep_val
        except ValueError:
            raise SystemExit(
                f"Config error: REPO_COMPACT_TOOL_KEEP must be an integer, got: {compact_tool_keep!r}"
            )
