...
import repo_claude
from repo_claude.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from repo_claude.core.permissions.storage import load_policy_file
from repo_claude.core.runner import AgentRunner
from repo_claude.core.runs import events_file, new_run_id
from repo_claude.core.session import SessionManager, SessionStore
from repo_claude.core.trace.record import TraceRecord
from repo_claude.core.trace.writer import TraceWriter
from repo_claude.core.transport.ipc_broadcaster import IpcEventBroadcaster
from repo_claude.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=repo_claude.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            sessions_root = os.environ.get("REPO_SESSIONS_ROOT", "./sessions")
            for candidate in Path(sessions_root).expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.repo/policy.toml").expanduser()
        ...
        # 注册退出信号处理器；Windows 上不支持 add_signal_handler，回退到 signal.signal
        try:
            loop.add_signal_handler(signal.SIGINT, shutdown.set)
            loop.add_signal_handler(signal.SIGTERM, shutdown.set)
        except NotImplementedError:
            signal.signal(signal.SIGINT, lambda _sig, _frame: shutdown.set())
